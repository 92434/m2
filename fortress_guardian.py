...
class FortressGuardian:
    ...
    def _activate_firewall(self) -> bool:
        ...
        try:
            # 模拟防火墙规则应用
            rules = self.config.get('security', {}).get('firewall', {}).get('rules', [])
        ...
        while self.status != "SHUTDOWN":
            try:
                ...
                if health_data['cpu_usage'] > 85:
                    self.logger.warning(f"⚠️ CPU使用率过高: {health_data['cpu_usage']}%")
                
                if health_data['memory_usage'] > 80:
                    self.logger.warning(f"⚠️ 内存使用率过高: {health_data['memory_usage']}%")
