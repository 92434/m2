...
class FortressConsole:
    ...
    def draw_header(self):
        ...
        # 绘制状态栏
        status_text = f"状态: OPERATIONAL | 时间: {datetime.now().strftime('%H:%M:%S')} | 视图: {self.current_view.upper()}"
    ...
    def draw_dashboard(self):
        ...
        overview_items = [
            ("CPU使用率", f"{stats.get('cpu', 0):.1f}%", self.get_status_color(stats.get('cpu', 0))),
            ("内存使用率", f"{stats.get('memory', 0):.1f}%", self.get_status_color(stats.get('memory', 0))),
            ("磁盘使用率", f"{stats.get('disk', 0):.1f}%", self.get_status_color(stats.get('disk', 0))),
        ...
        for module_name, status in modules.items():
            status_color = curses.color_pair(1) if status == 'active' else curses.color_pair(2)
        ...
        if self.alerts:
            y_pos += 1
            self.screen.addstr(y_pos, 2, "🚨 最新警报", curses.color_pair(2) | curses.A_BOLD)
            y_pos += 2
    ...
    def draw_module_management(self):
        ...
        y_pos = 4
        self.screen.addstr(y_pos, 2, "🔧 模块管理系统", curses.color_pair(1) | curses.A_BOLD)
        y_pos += 2
        ...
        for i, (name, status, priority) in enumerate(modules):
            ...
            status_color = curses.color_pair(1) if status == 'active' else curses.color_pair(3)
